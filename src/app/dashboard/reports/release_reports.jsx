...
const getSortLabel = () => {
  ...
  // Change the loadReportData function
const loadReportData = async () => {
  try {
    ...
    // Change to released blood reports endpoint
    const reports = await window.electronAPI.getAllReleasedBloodReports();

    if (reports.length === 0) {
      console.log("No released blood reports found, generating all historical reports...");
      await window.electronAPI.generateAllHistoricalReleasedBloodReports();
      const newReports = await window.electronAPI.getAllReleasedBloodReports();
      setReportData(formatReportData(newReports));
    } else {
      setReportData(formatReportData(reports));
    }
  } catch (err) {
    console.error("Error loading released blood report data:", err);
    setError(`Failed to load released blood report data: ${err.message}`);
  } finally {
    setLoading(false);
  }
};
  ...
  return reports.map((report) => {
    ...
    // Parse month data - released blood has single structure (no mobile/walkIn split)
    const month1Data =
...
const filteredReports = getSortedAndFilteredData();
  ...
  try {
    ...
    // Regenerate all historical released blood reports
    await window.electronAPI.generateAllHistoricalReleasedBloodReports();
    await loadReportData();
    setError(null);
  } catch (err) {
    console.error("Error refreshing released blood reports:", err);
    setError(`Failed to refresh released blood reports: ${err.message}`);
  } finally {
    setLoading(false);
  }
};
  ...
  const drawBarChart = (canvasRef, data, monthLabels) => {
    ...
    // Find max value and add buffer (extra 1-3 units above highest point)
    const maxValue = Math.max(...data.flatMap(d => [d.month1, d.month2, d.month3]));
    const buffer = Math.ceil(maxValue * 0.1) || 3; // 10% buffer or minimum 3
    ...
    ctx.fillText('Monthly Blood Type Distribution', width / 2, 30);
    ...
    ctx.fillStyle = '#000';
    ctx.fillText('Units Released', 0, 0);
    ctx.restore();
    ...
    const colors = [
      '#4D86EE',   // Month 1 - Blue
      '#FF8C00',   // Month 2 - Orange  
      '#2EB82E',   // Month 3 - Green
    ];
  ...
  // Update chart when preview report changes
  useEffect(() => {
  ...
  const drawBarChartPDF = (doc, data, monthLabels, startY) => {
    ...
    doc.text("Monthly Blood Type Distribution", chartX + chartWidth / 2, startY, { align: "center" });
    ...
    // Colors for the three months
    const colors = [
      [77, 134, 238],   // Month 1 - Blue
      [255, 140, 0],    // Month 2 - Orange
      [46, 184, 46],    // Month 3 - Green
    ];
    ...
    doc.setFontSize(8);
    doc.setTextColor(0, 0, 0);
    doc.text("Units Released", chartX - 0.4, axisStartY + chartHeight / 2, { 
      align: "center",
      angle: 90 
    });
...
doc.line(
  ...
  doc.setFontSize(11);
  doc.text(
    "RELEASED BLOOD DOCUMENTATION REPORT",
    4.25,
    2.5,
    { align: "center" }
  );
  ...
  const chartY = doc.lastAutoTable.finalY + 0.3;
    ...
    doc.save(`Released-Blood-Report-${previewReport.quarter}-${previewReport.year}.pdf`);
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        <h1 style={styles.title}>Released Blood Report</h1>
        <p style={styles.subtitle}>Released Blood Metrics & Analytical Overview</p>
      </div>
          ...
          >
            <div style={styles.modalHeader}>
              <div>
                ...
                <p style={styles.modalSubtitle}>
                  Released Blood Metrics & Analytical Overview
                </p>
              </div>
              <button
            ...
            <div style={styles.modalContent}>
              <div style={styles.previewContainer}>
                ...
                {/* Replace the two table sections with single table in preview */}
                <table style={styles.reportTable}>
                  ...
                  <tbody>
                    {["O+", "A+", "B+", "AB+", "O-", "A-", "B-", "AB-"].map((type) => {
                      ...
                      const total = previewReport.statistics[type] || 0;
