...
const NotificationComponent = () => {
  ...
  useEffect(() => {
    loadNotifications();
    // Poll for new notifications every 60 seconds (reduce spam)
    let lastUnread = 0;
  ...
  const getTypeIcon = (type) => {
    switch (type) {
      ...
      case 'upcoming_event':
      case 'event_finished': return <Calendar size={16} />;
  ...
  const getFilteredNotifications = () => {
    ...
    if (activeFilter === 'stock_expiration') {
      ...
      if (activeFilter === 'unread') {
        ...
        filtered = filtered.filter(n => n.type === 'upcoming_event' || n.type === 'event_finished');
  ...
  const getCounts = () => {
    return {
      ...
      upcoming_event: notifications.filter(n => n.type === 'upcoming_event' || n.type === 'event_finished').length,
