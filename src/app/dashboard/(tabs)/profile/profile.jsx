...
import { User, Clock, Settings, Camera, Upload, Trash2, ChevronDown, X, ZoomIn, ZoomOut, Users } from 'lucide-react';
import ProfileActivity from '../profile/profile_activity';
import ProfileSettings from '../profile/profile_settings';
import AccessControl from '../profile/access_control';
import UserApproval from '../profile/user_approval';
...
const ProfileComponent = () => {
  ...
  const [userManagementTab, setUserManagementTab] = useState('accessControl');
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      try {
        ...
        // Get original profile data to determine updated fields
        const originalProfile = await window.electronAPI.getUserProfileRBC(currentUser.userId);
        const updatedFields = [];

        if (profileData.profilePhoto !== originalProfile?.profile_photo) updatedFields.push('profilePhoto');
        if (profileData.dohId !== originalProfile?.doh_id) updatedFields.push('dohId');
        if (profileData.gender !== originalProfile?.gender) updatedFields.push('gender');
        if (profileData.dateOfBirth !== originalProfile?.date_of_birth) updatedFields.push('dateOfBirth');
        if (profileData.nationality !== originalProfile?.nationality) updatedFields.push('nationality');
        if (profileData.civilStatus !== originalProfile?.civil_status) updatedFields.push('civilStatus');
        if (profileData.bloodType !== originalProfile?.blood_type) updatedFields.push('bloodType');
        if (profileData.barangay !== originalProfile?.barangay) updatedFields.push('barangay');
        if (profileData.role !== originalProfile?.role) updatedFields.push('role');
        if (profileData.fullName !== originalProfile?.full_name) updatedFields.push('fullName');

        await window.electronAPI.logActivity({
          user_name: userName,
          action_type: 'update',
          entity_type: 'user_profile',
          entity_id: currentUser.userId.toString(),
          action_description: `${userName} updated their profile information`,
          details: {
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'userManagement':
        if (userManagementTab === 'accessControl') {
              ...
              <>
                ...
                >
                  ...
                  <ChevronDown
                    ...
                    style={{
                      marginLeft: 'auto',
                      transform: showUserManagement ? 'rotate(180deg)' : 'rotate(0deg)',
                  ...
                  <div style={{ paddingLeft: '48px' }}>
                    <button
                      onClick={() => {
                        ...
                        setUserManagementTab('accessControl');
                      }}
                      style={{
                        ...styles.navButton,
                        ...(activeTab === 'userManagement' && userManagementTab === 'accessControl' ? styles.navButtonActive : {}),
                        paddingLeft: '24px'
                      }}
                      onMouseEnter={(e) => {
                        if (!(activeTab === 'userManagement' && userManagementTab === 'accessControl')) {
                          Object.assign(e.target.style, styles.navButtonHover);
                        }
                      }}
                      onMouseLeave={(e) => {
                    ...
                    <button
                      onClick={() => {
                        setActiveTab('userManagement');
                        setUserManagementTab('userApproval');
                      }}
                      style={{
                        ...styles.navButton,
                        ...(activeTab === 'userManagement' && userManagementTab === 'userApproval' ? styles.navButtonActive : {}),
                        paddingLeft: '24px'
                      }}
                      onMouseEnter={(e) => {
                        if (!(activeTab === 'userManagement' && userManagementTab === 'userApproval')) {
                          Object.assign(e.target.style, styles.navButtonHover);
                        }
                      }}
                      onMouseLeave={(e) => {
