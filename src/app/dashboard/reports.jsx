...
const Reports = () => {
  ...
  const toggleAllSelection = () => {
    const allSelected = reportData.length > 0 && reportData.every((item) => item.selected);
    setReportData((prevData) =>
  ...
  const deleteSelectedItems = () => {
    const selectedIds = reportData.filter((item) => item.selected).map((item) => item.id);
  ...
  const allSelected =
    reportData.length > 0 && reportData.every((item) => item.selected);
  const someSelected =
    reportData.some((item) => item.selected) && !allSelected;
            ...
            <div style={styles.modalContent}>
              <div style={styles.previewContainer}>
                ...
                <div style={{ fontSize: "10px", marginBottom: "10px" }}>
                  Center for Health Development for: __________ Qtr: {previewReport.quarterNumber} Year: {previewReport.year}
                </div>
