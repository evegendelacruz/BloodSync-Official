...
import { Search, RefreshCw, Filter, User, Calendar, UserPlus, Edit, Trash2, CheckCircle, Clock, Eye } from 'lucide-react';
...
const RecentActivityOrg = () => {
  ...
  const loadActivities = async () => {
    try {
      ...
      if (typeof window !== 'undefined' && window.electronAPI) {
        const activitiesData = await window.electronAPI.getAllActivities(100, 0);
        ...
        setActivities([
          {
            ...
            entity_id: '123',
            action_description: 'Scheduled new appointment "Blood Drive Partnership - Santos" for September 25, 2024 at 9:00 AM',
            details: {
              appointmentTitle: 'Blood Drive Partnership - Santos',
  ...
  const getActivityColor = (actionType) => {
    switch (actionType) {
      ...
      case 'complete':
        return '#8b5cf6'; // Purple for completions
      default:
  ...
  const activityStats = {
    ...
    appointments: activities.filter(a => a.entity_type === 'appointment').length
  };
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.statsContainer}>
        ...
        <div style={styles.statCard}>
          <div style={styles.statNumber}>{activityStats.appointments}</div>
          <div style={styles.statLabel}>Appointment Actions</div>
        </div>
      </div>
                  ...
                  >
                    ...
                    <div style={styles.activityContent}>
                      <div style={styles.activityDetails}>
                        <div style={styles.userName}>{activity.user_name}</div>
                        <div style={styles.actionText}>{activity.action_description}</div>
                        <div style={styles.entityBadge}>
                          {activity.entity_type === 'donor' ? 'Donor Management' : 'Appointment Management'}
                        </div>
                      </div>
