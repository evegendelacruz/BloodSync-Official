...
import DiscardedInvoice from "./invoice/discarded_blood";
...
const DashboardContent = ({ onNavigate }) => {
    ...
    const fetchBloodCounts = async () => {
      ...
      try {
        ...
        try {
          ...
          // Combine both blood stocks and Non-Conforming Staffs, limit to top 5
          const allExpiring = [
  ...
  // Stock Level Monitoring - Check for low/out of stock
  useEffect(() => {
    const checkStockLevels = async () => {
      try {
        if (window.electronAPI && window.electronAPI.checkAndCreateStockLevelNotifications) {
          const result = await window.electronAPI.checkAndCreateStockLevelNotifications();
          if (result && result.notificationsCreated > 0) {
            console.log(`[Dashboard] Created ${result.notificationsCreated} stock level notifications`);
            // Refresh notification count after creating new notifications
            try {
              const notifCount = await window.electronAPI.getUnreadNotificationCount();
              setUnreadNotificationCount(notifCount || 0);
              // Dispatch global event for notification update
              window.dispatchEvent(new Event('bloodsync-notifications-updated'));
            } catch (error) {
        ...
        console.error('[Dashboard] Error checking stock levels:', error);
      }
    };

    // Check stock levels immediately on mount
    checkStockLevels();

    // Then check every 5 minutes (300000ms)
    const stockCheckInterval = setInterval(checkStockLevels, 300000);
  ...
  const getEventStatus = (dateStr, status) => {
    ...
    // Use 'confirmed' for the "upcoming" status to match calendar.jsx
    return eventDate < today ? 'finished' : 'confirmed'; 
  };

  const getEventStatusColor = (status, dateString) => {
    // Get the dynamic status (e.g., 'finished', 'upcoming')
    const dynamicStatus = getEventStatus(dateString, status);
    
    switch (dynamicStatus) {
      case 'confirmed': return '#3b82f6'; // Blue (Upcoming/Scheduled)
      case 'finished': return '#10b981';  // Green (Finished)
  ...
  // In dashboard.jsx, around line 220
  useEffect(() => {
    const fetchUpcomingDrives = async () => {
      try {
        console.log('[Dashboard] Fetching upcoming drives...');

        // 1. Fetch partnership requests from RBC DB
        const partnershipRequests = await window.electronAPI.getAllPartnershipRequests(null);
        console.log('[Dashboard] Partnership requests:', partnershipRequests);

        // 2. Fetch local appointments from Org DB - FIXED: Use getAllAppointments instead
        const localAppointments = await window.electronAPI.getAllAppointments();
        console.log('[Dashboard] Local appointments:', localAppointments);

        const today = new Date();
        today.setHours(0, 0, 0, 0);

        // 3. Map and filter partnership requests
        ...
        // 4. Map and filter local appointments
...
const Dashboard = () => {
  ...
  const hasAccessToFeature = (feature) => {
    ...
    const rolePermissions = {
      'Admin': ['mail', 'calendar', 'notification'],
      'Non-Conforming Staff': ['mail', 'calendar', 'notification'],
      'Inventory Staff': ['mail', 'calendar', 'notification'],
      'Scheduler': ['mail', 'calendar', 'notification']
    };
  ...
  useEffect(() => {
    const handleNotificationCountUpdate = async () => {
      try {
        if (window.electronAPI && window.electronAPI.getUnreadNotificationCount) {
          const count = await window.electronAPI.getUnreadNotificationCount();
          setUnreadNotificationCount(count || 0);
        ...
        console.error('[Dashboard] Error updating notification count:', error);
      }
    };
    window.addEventListener('bloodsync-notifications-updated', handleNotificationCountUpdate);
    return () => {
      window.removeEventListener('bloodsync-notifications-updated', handleNotificationCountUpdate);
    };
  }, []);
...
const getNotificationMainIcon = (notification) => {
  ...
  if (type === 'stock_expired') {
    return <img src="/assets/expired-blood.png" alt="Expired Blood" style={{ width: '20px', height: '20px' }} />;
  }

  // Stock OUT notifications (critical)
  if (type === 'stock_out') {
    return <img src="/assets/expired-blood.png" alt="Out of Stock" style={{ width: '20px', height: '20px' }} />;
  }
...
const getNotificationIconStyle = (notification) => {
  ...
  const markAllAsRead = async () => {
    try {
      // First, call the backend API to mark all notifications as read in the database.
      if (typeof window !== 'undefined' && window.electronAPI) {
        // Persist backend notifications
        await window.electronAPI.markAllNotificationsAsRead();

        // Persist locally-generated finished event notifications
        const currentIds = notifications.map(n => n.id);
        const existing = JSON.parse(localStorage.getItem('rbcReadNotificationIds') || '[]');
        const mergedIds = Array.from(new Set([...existing, ...currentIds]));
        localStorage.setItem('rbcReadNotificationIds', JSON.stringify(mergedIds));
      }

      // After the backend operation is successful, reload the notifications from the database.
      // This ensures the UI state is a true reflection of the backend data.
      await loadNotifications();

    } catch (error) {
      console.error('Error marking all notifications as read:', error);
      // If an error occurs, still try to reload notifications to prevent UI inconsistencies.
      await loadNotifications();
    }
  };

  // Note: "Mark all as read" functionality is for notifications, not mail.
  ...
  const loadNotifications = useCallback(async () => {
    try {
      if (typeof window !== 'undefined' && window.electronAPI) {
        ...
        const transformedNotifications = notificationsData.map(n => {
          ...
          if (n.link_to) {
            try {
              ...
              console.warn('[Dashboard] Failed to parse stock data for notification', n.id, ':', e);
            }
          }

          // Map priority to status for expiration notifications
          let displayStatus = n.status || 'unread';
        ...
        const now = new Date();
        const readLocalIds = JSON.parse(localStorage.getItem('rbcReadNotificationIds') || '[]');
        const finishedEventNotifications = appointmentsData
          .filter(apt => {
            const endOfDay = new Date(apt.date + 'T23:59:59');
            const diffMs = now - endOfDay;
            return diffMs >= 0 && diffMs <= 10 * 60 * 1000 && (apt.status === 'approved' || apt.status === 'confirmed');
          })
          .map(apt => ({
            ...
            timestamp: new Date(),
            read: readLocalIds.includes(`event-finished-${apt.id || apt.appointment_id}`)
          }));
        ...
        const testFinished = {
          ...
          status: 'info',
          title: 'Blood Drive Partnership event now Finished',
          message: 'The event "Blood Drive Partnership" has ended at 12:00 midnight.',
          requestor: 'Event Reminder',
          timestamp: new Date(),
          read: readLocalIds.includes('event-finished-test')
        };

        const merged = [...transformedNotifications, ...finishedEventNotifications, testFinished];
        ...
        setNotifications([
          {
            ...
            message: 'Santos (Barangay) requested a blood drive partnership for 2025-09-25 at 9:00 AM.',
          ...
          },
          {
            ...
            status: 'pending',
            title: 'Blood Drive Partnership Request',
            message: 'Cruz (Organization) requested a blood drive partnership for 2025-09-28 at 2:00 PM.',
          ...
          {
            id: 3,
            notificationId: 'EVENT-FINISHED-DEMO',
            type: 'upcoming_event',
            status: 'info',
            title: 'Blood Drive Partnership event now Finished',
            message: 'The event "Blood Drive Partnership" has ended at 12:00 midnight.',
  ...
  const getLatestNotifications = () => {
    return notifications
      .filter(n => {
        // Filter out donor records sync request approval notifications
        if (
          n.title?.includes('Donor Records Sync Request Approval') ||
          n.title?.includes('Donor Record Sync Request Approval') ||
          n.description?.includes('requested to sync') ||
          n.description?.includes('Please review and approve the sync request') ||
  ...
  useEffect(() => {
    loadNotifications();
    // Change interval to once every 24 hours (86,400,000 ms)
    const notifInterval = setInterval(loadNotifications, 86400000);
  ...
  const loadMailMessages = async () => {
    try {
      ...
      if (typeof window !== 'undefined' && window.electronAPI) {
        ...
        const mailsFromSyncRequests = syncRequests.map(s => {
          const avatar = s.requestor_name ? s.requestor_name.split(' ').map(name => name[0]).join('').substring(0, 2).toUpperCase() : 'SR';
          const avatarColors = ['#3b82f6', '#10b981', '#8b5cf6', '#f59e0b', '#ef4444'];
          const avatarColor = avatarColors[Math.floor(Math.random() * avatarColors.length)];

          return {
            id: `sync-${s.requestor_name}-${s.organization}`,
            from: `${s.requestor_name} (${s.organization})`,
            subject: `Donor Records Sync Request`,
            preview: `${s.requestor_name} from ${s.organization} has requested to sync ${s.pending_count} donor record(s).`,
            avatar: avatar,
            avatarColor: avatarColor,
            read: readMailIds.includes(`sync-${s.requestor_name}-${s.organization}`),
            status: 'pending',
            type: 'sync'
          };
        });
  ...
  const loadCalendarAppointments = async () => {
    try {
      ...
      // 4. Map and filter local appointments - FIX THE MAPPING
      const upcomingLocal = localAppointments
          .filter(apt => {
            const eventDate = new Date(apt.date + 'T00:00:00');
            return (apt.status === 'approved' || apt.status === 'confirmed' || apt.status === 'scheduled') && eventDate >= today;
        ...
        .map(apt => ({
          id: apt.id || apt.appointment_id,
          event_date: apt.date, // Use 'date' field
          event_time: apt.time, // Use 'time' field
          organization_name: apt.title || `Blood Drive - ${apt.contactInfo?.lastName || 'Event'}`,
          event_address: apt.contactInfo?.address || apt.address, // Add fallback
          organization_barangay: apt.contactInfo?.barangay || '', 
          status: apt.status
        }));
      ...
      console.log('[Dashboard] Loaded calendar appointments:', allUpcoming);
    } catch (error) {
  ...
  switch (activeScreen) {
    case "mail":
      return <MailComponent onNavigate={handleNavigate} />;
    case "calendar":
  ...
  return (
    <div style={styles.dashboardContainer}>
        ...
        <nav style={styles.navBar}>
          <div style={styles.navContent}>
            ...
            <div style={styles.navRight}>
              ...
              {hasAccessToFeature('notification') && (
              <div style={styles.dropdownContainer}>
                <button
                  style={{
                    ...styles.navButton,
                          ...
                          <button
                            ...
                            onClick={async () => {
                              try {
                                ...
                                console.error('Error running expiration check:', e);
                              }
                            }}
      ...
      <style>{`
        ...
        .icon-circle {
          width: 32px;  /* The main green circle */
          height: 32px;
          border-radius: 50%;
          display: flex;
          align-items: center;
        ...
        .icon-green-bg {
          background-color: #f3f4f6;
          border-radius: 50%;
          width: 20px;  /* The green circle inside */
          height: 20px;
          display: flex;
          align-items: center;
          justify-content: center;
        ...
        .dropdown-blood-icon {
          width: 20px;   /* <-- This is the fix: constrains the image size */
          height: 20px;  /* <-- This is the fix: constrains the image size */
          border-radius: 50%;
          object-fit: contain; /* This ensures the image scales nicely */
        }
        .dropdown-item {
          padding: 0.875rem 1rem;
