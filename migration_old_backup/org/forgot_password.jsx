...
const ForgotPasswordOrg = () => {
  ...
  const closeModal = () => {
    ...
    if (modalInfo.type === "success") {
      ...
      sessionStorage.setItem('resetEmailOrg', formData.email);
      const currentTime = Date.now();
      sessionStorage.setItem('otpSentTimeOrg', currentTime.toString());
  ...
  const handleSendCode = async (e) => {
    ...
    try {
      ...
      const data = await window.electronAPI.generatePasswordResetTokenOrg(formData.email);
            ...
            <form onSubmit={handleSendCode}>
              ...
              {showGotOTP && (
                <div style={{ textAlign: "center", marginBottom: "15px" }}>
                  <button
                    type="button"
                    onClick={() => navigate("/organization/reset-password")}
                    className="link"
                    style={{
                      background: "none",
                      border: "none",
