...
// db_org will be loaded later in the setupDatabaseHandlers function
...
const setupIpcHandlers = () => {
  ...
  const handlersToRemove = [
    ...
    // ADD THESE (DONOR AND APPOINTMENT HANDLERS WITH ACTIVITY LOGGING)
    "db:getAllDonors",
    "db:addDonor",
    "db:updateDonor",
    "db:deleteDonors",
    "db:searchDonors",
    "db:getDonorByDonorId",
    "db:getDonorStatistics",
    "db:testDonorConnection"
  ];
  ...
  // Note: getAllNotificationsOrg handler is defined later in the organization section
  ...
  ipcMain.handle("db:checkAndCreateStockLevelNotifications", async () => {
    try {
      return await dbService.checkAndCreateStockLevelNotifications();
    } catch (error) {
      console.error("IPC Error - checkAndCreateStockLevelNotifications:", error);
      throw error;
    }
  });
  ...
  // ========== USER PERMISSIONS IPC HANDLERS ==========
  ipcMain.handle("db:getUserPermissions", async (_event, userId) => {
    try {
      return await dbService.getUserPermissions(userId);
    } catch (error) {
      console.error("IPC Error - getUserPermissions:", error);
      throw error;
    }
  });

  ipcMain.handle("db:updateUserPermissions", async (_event, userId, permissions) => {
    try {
      return await dbService.updateUserPermissions(userId, permissions);
    } catch (error) {
      console.error("IPC Error - updateUserPermissions:", error);
      throw error;
    }
  });
...
// These use dbOrgService to read from notifications_org table
...
ipcMain.handle("db:getAllNotificationsOrg", async () => {
  try {
    console.log('[IPC] getAllNotificationsOrg handler called');
    const result = await dbOrgService.getAllNotificationsOrg();
    console.log('[IPC] getAllNotificationsOrg returned', result?.length || 0, 'notifications');
    return result;
  } catch (error) {
...
ipcMain.handle("db:deleteNotificationOrg", async (_event, notificationId) => {
  ...
  // Password and Phone Number Management - RBC

  ipcMain.handle("db:verifyPasswordRBC", async (_event, userId, password) => {
    try {
      // FIX: Was dbService.verifyPassword
      return await dbService.verifyPasswordRBC(userId, password); 
    } catch (error) {
      console.error("IPC Error - verifyPasswordRBC:", error);
      throw error;
    }
  });

  ipcMain.handle("db:updateUserPasswordRBC", async (_event, userId, newPassword) => {
    try {
      // FIX: Was dbService.updateUserPassword
      return await dbService.updateUserPasswordRBC(userId, newPassword);
    } catch (error) {
      console.error("IPC Error - updateUserPasswordRBC:", error);
      throw error;
    }
  });

  ipcMain.handle("db:sendPasswordResetOTPRBC", async (_event, email) => {
    try {
      // FIX: Was dbService.sendPasswordResetOTP
      return await dbService.sendPasswordResetOTPRBC(email);
    } catch (error) {
      console.error("IPC Error - sendPasswordResetOTPRBC:", error);
      throw error;
    }
  });

  ipcMain.handle("db:verifyPasswordResetOTPRBC", async (_event, email, otp) => {
    try {
      // FIX: Was dbService.verifyPasswordResetOTP
      return await dbService.verifyPasswordResetOTPRBC(email, otp);
    } catch (error) {
      console.error("IPC Error - verifyPasswordResetOTPRBC:", error);
      throw error;
    }
  });

  ipcMain.handle("db:updatePhoneNumberRBC", async (_event, userId, phoneNumber) => {
    try {
      // FIX: Was dbService.updatePhoneNumber
      return await dbService.updatePhoneNumberRBC(userId, phoneNumber);
    } catch (error) {
      console.error("IPC Error - updatePhoneNumberRBC:", error);
      throw error;
    }
  });

  ipcMain.handle("db:sendPhoneVerificationOTPRBC", async (_event, phoneNumber) => {
    try {
      // FIX: Was dbService.sendPhoneVerificationOTP
      return await dbService.sendPhoneVerificationOTPRBC(phoneNumber);
    } catch (error) {
      console.error("IPC Error - sendPhoneVerificationOTPRBC:", error);
      throw error;
    }
  });

  ipcMain.handle("db:verifyPhoneOTPRBC", async (_event, phoneNumber, otp) => {
    try {
      // FIX: Was dbService.verifyPhoneOTP
      return await dbService.verifyPhoneOTPRBC(phoneNumber, otp);
    } catch (error) {
      console.error("IPC Error - verifyPhoneOTPRBC:", error);
      throw error;
    }
  });

  ipcMain.handle("db:updateEmailRBC", async (_event, userId, newEmail) => {
    try {
      return await dbService.updateEmailRBC(userId, newEmail);
    } catch (error) {
      console.error("IPC Error - updateEmailRBC:", error);
      throw error;
    }
  });

  // Password and Phone Number Management - ORG
  ipcMain.handle("db:verifyPassword", async (_event, userId, password) => {
    try {
      return await dbOrgService.verifyPassword(userId, password);
    } catch (error) {
      console.error("IPC Error - verifyPassword:", error);
      throw error;
    }
  });

  ipcMain.handle("db:updateUserPassword", async (_event, userId, newPassword) => {
    try {
      return await dbOrgService.updateUserPassword(userId, newPassword);
    } catch (error) {
      console.error("IPC Error - updateUserPassword:", error);
      throw error;
    }
  });

  ipcMain.handle("db:sendPasswordResetOTP", async (_event, email) => {
    try {
      return await dbOrgService.sendPasswordResetOTP(email);
    } catch (error) {
      console.error("IPC Error - sendPasswordResetOTP:", error);
      throw error;
    }
  });

  ipcMain.handle("db:verifyPasswordResetOTP", async (_event, email, otp) => {
    try {
      return await dbOrgService.verifyPasswordResetOTP(email, otp);
    } catch (error) {
      console.error("IPC Error - verifyPasswordResetOTP:", error);
      throw error;
    }
  });

  ipcMain.handle("db:updatePhoneNumber", async (_event, userId, phoneNumber) => {
    try {
      return await dbOrgService.updatePhoneNumber(userId, phoneNumber);
    } catch (error) {
      console.error("IPC Error - updatePhoneNumber:", error);
      throw error;
    }
  });

  ipcMain.handle("db:sendPhoneVerificationOTP", async (_event, phoneNumber) => {
    try {
      return await dbOrgService.sendPhoneVerificationOTP(phoneNumber);
    } catch (error) {
      console.error("IPC Error - sendPhoneVerificationOTP:", error);
      throw error;
    }
  });

  ipcMain.handle("db:verifyPhoneOTP", async (_event, phoneNumber, otp) => {
    try {
      return await dbOrgService.verifyPhoneOTP(phoneNumber, otp);
    } catch (error) {
      console.error("IPC Error - verifyPhoneOTP:", error);
      throw error;
    }
  });
...
// ========== SYNC REQUEST IPC HANDLERS ==========
  ...
  // Update sync request status (approve/decline individual request)
  ipcMain.handle('db:updateSyncRequestStatus', async (event, organization, userName, status, by, reason = null) => {
    try {
      const result = await dbService.updateSyncRequestStatus(organization, userName, status, by, reason);
      console.log(`[IPC] Sync request for ${organization}/${userName} updated to ${status}`);
      return result;
    } catch (error) {
      console.error('Error in updateSyncRequestStatus:', error);
      throw error;
    }
  });
  ...
  ipcMain.handle('db:approveDonorSync', async (event, approvedBy) => {
    try {
      // Get pending requests before approving to know which orgs to notify
      const pendingRequests = await dbService.getPendingSyncRequests();

      const result = await dbService.approveDonorSync(approvedBy);

      // Create notifications for each organization that requested sync
      const uniqueOrgs = [...new Map(pendingRequests.map(req =>
        [req.source_user_id, { userId: req.source_user_id, userName: req.source_user_name, organization: req.source_organization }]
      )).values()];

      for (const org of uniqueOrgs) {
        try {
          await dbService.createOrgNotification({
            userId: org.userId,
            userName: approvedBy,
            notificationType: 'sync_approved',
            title: 'Donor Records Sync Request is now Approved',
            description: `${approvedBy} from Regional Blood Center has approved your donor records sync request. ${result.totalProcessed} donor record(s) have been successfully synced to the main server.`,
            relatedEntityType: 'donor_sync',
            relatedEntityId: null,
            linkTo: 'donor-record',
            priority: 'high'
          });
        } catch (notifErr) {
          console.error(`Error creating notification for org ${org.organization}:`, notifErr);
        }
      }
...
// END : ALL METHODS MAADE BY CHRISTIAN PAASA ============================================================================================
...
app.whenReady().then(() => {
  ...
  try {
    ...
    dbService.checkAndCreateExpirationNotifications().catch(() => {});
    dbService.checkAndCreateStockLevelNotifications().catch(() => {}); // ADD THIS
    dbOrgService.checkForFinishedAppointmentsAndNotify().catch(() => {}); // Run on startup for org appointments
    dbOrgService.checkForUpcomingTomorrowAppointments().catch(() => {}); // Check for tomorrow's appointments

    setInterval(() => {
      dbService.checkAndCreateExpirationNotifications().catch(() => {});
      dbService.checkAndCreateStockLevelNotifications().catch(() => {}); // ADD THIS
      dbOrgService.checkForFinishedAppointmentsAndNotify().catch(() => {}); // Also check for finished org appointments
      dbOrgService.checkForUpcomingTomorrowAppointments().catch(() => {}); // Check for tomorrow's appointments
    }, 60 * 60 * 1000); // Check every hour for all automated tasks
  } catch (_) {}
